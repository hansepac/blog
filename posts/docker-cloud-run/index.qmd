...
To add OAuth, move onto [**OAuth with Identiy-Aware Proxy**](#oauth-with-identity-aware-proxy).
...
Now, navigate to Cloud Run and follow the same steps as shown in the [continous deployment method](#continuous-deployment-via-github), except this time we will select our image from the Artifact Registry with the "Deploy one revision from an existing container image" option.
